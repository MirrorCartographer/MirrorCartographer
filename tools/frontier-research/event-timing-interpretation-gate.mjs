...
export function evaluateEventTimingInterpretation(input = {}, options = {}) {
  ...
  const result = {
    ...
    reason: '',
    epistemic_limit: 'Structural replay validity and observation coverage are necessary but do not prove event delivery, responsiveness quality, replay completeness, visual divergence, or causality.',
    privacy_boundary: {
      retains: ['opaque navigation id', 'layer validity flags', 'coverage class', 'reason code'],
      excludes: ['raw timing entries', 'event names', 'event targets', 'selectors', 'raw timestamps', 'time origins', 'user-agent strings']
    }
  };
  ...
  result.reason = 'replay structure and same-window observation coverage independently passed';
  return result;
}
