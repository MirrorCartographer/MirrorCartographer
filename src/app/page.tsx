...
function createMap({
  ...
  const hasBody = Boolean(bodyArea) || includesAny(combined, ["body", "chest", "eye", "stomach", "heart", "pain", "breath", "dizzy", "nervous", "somatic", "jaw", "ribs", "spine"]);
  const hasCertainty = includesAny(combined, ["certainty", "proof", "prove", "real", "true", "truth", "sure", "guarantee"]);
  ...
  const modeInterpretation = {
    ...
    Reflective:
      `Reflective mode: compare the current signal with user-provided history and echo patterns${memoryEnabled ? " using the optional memory/echo layer" : " without claiming memory beyond this session"}.`,
    Mythopoetic:
  ...
  return {
    ...
    audit: `Type tag: ${mode}. Reflection classifier: ${mode === "Mythopoetic" ? "speculative/poetic" : mode === "Canonical" ? "source-bounded" : "personal-echo"}. Hallucination audit: mark all uncertain claims and do not invent sources.`,
...
export default function Home() {
  ...
  const sections = [
    ...
    ["Mode interpretation", map.interpretation],
    ["Audit tag", map.audit],
      ...
      <section className="mx-auto flex w-full max-w-7xl flex-col gap-10 px-6 py-10 md:px-10 lg:px-16">
        <header className="rounded-[2rem] border border-[#745a7d]/40 bg-[#1b1620] p-8 shadow-2xl shadow-black/30">
          ...
          <p className="mt-6 max-w-4xl text-lg leading-8 text-[#d8cfdc]">
            This public demo turns body area, color, texture, symbol, and narrative input into a reviewable reflection map. It demonstrates the controlled architecture: Canonical, Reflective, and Mythopoetic modes with visible audit labels.
          </p>
        </header>
        ...
        <section className="grid gap-6 xl:grid-cols-[0.85fr_1.15fr]">
          <div className="rounded-[1.5rem] border border-[#745a7d]/40 bg-[#17121c] p-6">
            ...
            <p className="mt-2 text-sm leading-6 text-[#cfc3d6]">
              This approximates the planned UI: body-map mark, symbolic inputs, scene entry, mode toggle, optional echo tracker, and audited output.
            </p>
            ...
            <div className="mt-5 rounded-2xl border border-[#745a7d]/40 bg-[#100c14] p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-3">
                {modes.map((item) => (
                  <button
                    key={item}
                    className={`rounded-full border px-4 py-2 text-sm ${mode === item ? "border-[#d7b7ff] bg-[#d7b7ff] text-[#170f1d]" : "border-[#745a7d]/50 text-[#eadcf3] hover:border-[#d7b7ff] hover:bg-[#261c2c]"}`}
