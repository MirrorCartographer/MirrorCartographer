...
export function buildWorkflowInvocationReceipt({ context, policy, generatedAt }) {
  ...
  const runAttempt = Number(context.run_attempt)
  const runNumber = Number(context.run_number)
  ...
  const exactFields = [
    ...
    field,
    expected: clean(policy[field]),
    actual: clean(context[field]),
    accepted: clean(policy[field]) !== '' && clean(context[field]) === clean(policy[field])
  }))
  ...
  const invocation = {
    ...
    repository_id: clean(context.repository_id),
    repository_owner_id: clean(context.repository_owner_id),
    workflow_ref: clean(context.workflow_ref),
    workflow_sha: clean(context.workflow_sha),
    ref: clean(context.ref),
    event_name: clean(context.event_name),
    environment: clean(context.environment),
    run_id: clean(context.run_id),
    run_attempt: runAttempt,
    run_number: runNumber,
    actor_id: clean(context.actor_id),
    triggering_actor: clean(context.triggering_actor),
    source_sha: clean(context.source_sha)
  }

  const replayKey = accepted
    ? crypto.createHash('sha256').update(canonicalize({
  ...
  return {
    ...
    accepted,
    reason: accepted ? 'workflow_context_bound' : 'workflow_context_rejected',
    missing_fields: missing,
    exact_fields: exactFields,
    structural_checks: {
      numeric_run_identity: numericIdentity,
      workflow_sha: workflowShaAccepted,
      source_sha: sourceShaAccepted,
      actor_identity: actorAccepted
    },
    invocation,
    replay_key_sha256: replayKey,
    source_status: 'github_actions_context_unverified_by_oidc',
...
function contextFromEnvironment(env) {
  return {
    ...
    environment: env.CLOUDFLARE_DEPLOYMENT_ENVIRONMENT,
