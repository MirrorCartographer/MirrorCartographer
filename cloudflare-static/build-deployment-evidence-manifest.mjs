...
function assertSafeRelativeFile(value) {
  if (typeof value !== 'string' || value.length === 0) throw new Error('invalid-evidence-path');
  if (path.isAbsolute(value) || value.includes('..') || value.includes('\\')) throw new Error(`unsafe-evidence-path:${value}`);
  if (path.basename(value) !== value) throw new Error(`nested-evidence-path:${value}`);
}
...
export function buildDeploymentEvidenceManifest({ directory='.', files=REQUIRED_EVIDENCE_FILES, sourceCommit, runId, generatedAt=new Date().toISOString() } = {}) {
  ...
  if (unique.size !== files.length) throw new Error('duplicate-evidence-path');
  ...
  const entries = [...files].sort().map((relativePath) => {
    ...
    if (bytes.length === 0) throw new Error(`empty-evidence-file:${relativePath}`);
    return { path: relativePath, sha256: sha256(bytes), size_bytes: bytes.length };
  });

  const manifest = {
    schema_version: '1.0.0',
    artifact_type: 'cloudflare-deployment-evidence-manifest',
  ...
  return { ...manifest, manifest_sha256: sha256(Buffer.from(canonical)) };
}

function main() {
  const [directory='.', output='cloudflare-deployment-evidence-manifest.json'] = process.argv.slice(2);
  const manifest = buildDeploymentEvidenceManifest({
    directory,
