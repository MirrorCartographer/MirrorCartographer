...
export async function runGitHubOidcEvidence({ audience = DEFAULT_AUDIENCE, env = process.env, fetchImpl = globalThis.fetch, nowEpochSeconds } = {}) {
  ...
  return {
    ...
    trust_limit: 'Acceptance proves a signed, fresh GitHub Actions identity matched this invocation. Replay rejection, Cloudflare deployment success, evidence truth, and scientific or medical truth remain separate.'
  }
}

async function main() {
  const [outputPath = 'cloudflare-github-oidc-evidence.json', audience = DEFAULT_AUDIENCE] = process.argv.slice(2)
  const result = await runGitHubOidcEvidence({ audience })
  fs.writeFileSync(outputPath, `${JSON.stringify(result, null, 2)}\n`, { mode: 0o600 })
  process.stdout.write(`${JSON.stringify({ accepted: result.accepted, reason: result.decision.reason, output: outputPath })}\n`)
