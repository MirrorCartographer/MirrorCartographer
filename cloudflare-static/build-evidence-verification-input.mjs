...
function traceparentFor(invocationId, digest) {
  ...
  const traceId = seed.slice(0, 32).replace(/^0+$/, '1'.padEnd(32, '0'));
  const parentId = seed.slice(32, 48).replace(/^0+$/, '1'.padEnd(16, '0'));
...
export function buildEvidenceVerificationInput({ proofText, proof, attestationBundle, signatureVerification = null }) {
  ...
  const envelope = {
    ...
    traceparent: traceparentFor(invocationId, digest),
    created_at: proof.generated_at ?? new Date(0).toISOString(),
    team: EXPECTED.team,
    queue_item: EXPECTED.queueItem,
    claim_state: claimVerified ? 'observed' : 'unresolved',
    evidence_state: claimVerified ? 'deployment-verified' : 'unverified',
    summary: claimVerified
      ? 'Cloudflare returned a deployment URL and the served identity verifier reported success.'
      : 'Cloudflare deployment evidence is incomplete or the served identity verifier did not report success.',
    sources: [{ repository: EXPECTED.sourceRepository, commit: proof.source_commit ?? null }],
    artifacts: [{ name: EXPECTED.artifactName, sha256: digest }],
    verification: verifierRows,
    falsification_routes: [
      'Recompute the proof SHA-256 and compare it with the attestation subject.',
      'Re-run gh attestation verify against the exact proof bytes and repository.',
      'Fetch the recorded deployment URL and rerun the served identity verifier.'
    ],
    limits: [
      'Artifact and provenance acceptance does not prove scientific truth.',
      'A deployment URL without successful served-identity verification is not deployment evidence.'
    ],
    peer_triggers: []
  };
  ...
  const policy = {
    schema_version: '1.0.0',
    enabled: true,
    allowed_builder_ids: [EXPECTED.builderId],
    allowed_source_repositories: [EXPECTED.sourceRepository],
    allowed_build_types: [EXPECTED.buildType],
    require_invocation_id: true
  };
  ...
  return {
    ...
    expected: {
      ...
      sourceCommit: proof.source_commit ?? null,
      team: EXPECTED.team,
      queueItem: EXPECTED.queueItem
    },
    diagnostics: {
      signatureVerification: normalizedSignature,
      subjectVerification: {
...
function main(argv = process.argv.slice(2)) {
  ...
  process.stdout.write(JSON.stringify({ output: outputPath, signature: result.signatureVerification.status, claim: result.diagnostics.claimEvidence.status }) + '\n');
}
